...
pub fn read_header(f: &mut File) -> io::Result<CartHeader> {
    ...
    macro_rules! buf_to_buf {
        ($other:expr, $range:expr) => {{
            ...
            $other.resize(range.len(), 0);
            $other.copy_from_slice(range);
        }};
    }
    ...
    buf_to_buf!(header.entry, ENTRY_POINT);
    buf_to_buf!(header.logo, NINTENDO_LOGO);
    header.title = buf_string!(TITLE);
    header.manufacturer = buf_string!(MANUFACTURER);
    header.cgb = buf_at!(CGB_FLAG);
    header.new_licensee = buf_string!(NEW_LICENSEE);
    header.sgb = buf_at!(SGB_FLAG) == 0x03;
    header.cart_type = match buf_at!(CART_TYPE) {
    ...
    header.rom_banks = match buf_at!(ROM_SIZE) {
        value if (value <= 8) => 1 << value,
        _ => 1, // Assume 1 for invalid bytes
    };
    header.ram_banks = match buf_at!(RAM_SIZE) {
        0x00 => 0,
        0x02 => 1,
        0x03 => 4,
        0x04 => 16,
    ...
    header.destination = buf_at!(DESTINATION);
    header.old_licensee = buf_at!(OLD_LICENSEE);
    header.version = buf_at!(VERSION);
    header.header_checksum = buf_at!(HEADER_CHECKSUM);
    header.global_checksum = make_word!(buf_at!(GLOBAL_CHECKSUM_H), buf_at!(GLOBAL_CHECKSUM_L));
